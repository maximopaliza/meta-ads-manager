"""
campaign_uploader.py — Procesa jobs de campaña pendientes del dashboard.

El dashboard crea la campaña y conjuntos en Meta, guarda el job en campaign_drafts
con status='pending_bot'. Este scheduler descarga los videos de Drive,
los sube a Meta y notifica por Telegram.
"""
import logging
import os
import tempfile
...
async def _process_job(job: dict, bot=None) -> None:
    job_id   = job['id']
    spec     = job.get('ads') or {}
    campaign_name = job.get('campaign_name', 'Campaña')
    ...
    for set_spec in ad_sets:
        ...
        for ad_spec in set_spec.get('ads', []):
            ...
            try:
                ...
                file_bytes, _ = await loop.run_in_executor(None, download_file, file_id)
                ...
                if is_video:
                    ...
                    story_spec = _build_video_story(page_id, video_id, headline, primary,
                                                    destination_url, cta, ad_description,
                                                    url_params, ig_account_id)
                else:
                    image_hash = await loop.run_in_executor(
                        None, _upload_image, account_id, file_bytes
                    )
                    story_spec = _build_image_story(page_id, image_hash, headline, primary,
                                                    destination_url, cta, ad_description,
                                                    url_params, ig_account_id)
    ...
    if bot:
        ...
        if chat_id:
            lines = [f"✅ <b>{campaign_name}</b> — ads subidos\n",
                     f"📊 {success_count}/{total_ads} ads creados en <b>PAUSED</b>"]
            if error_count:
                lines.append(f"⚠️ {error_count} fallaron:")
                for e in errors[:3]:
                    lines.append(f"  • {e}")
