    ...
    try {
      ...
      const res = await fetch('/api/campaign/create', {
        ...
        body: JSON.stringify({
          ...
          specialAdCategory: config.specialAdCategory,
          optimizationGoal:  config.optimizationGoal,
          pixelEvent:        config.pixelEvent,
          pixelId:           config.pixelId || undefined,
          bidStrategy:       config.bidStrategy,
          bidAmount:         config.bidAmount ? parseFloat(config.bidAmount) * 100 : undefined,
          billingEvent:      config.billingEvent,
          attributionWindow: config.attributionWindow,
          placements:        config.placements,
          platforms:         config.platforms,
          accountId:         config.accountId,
          pageId:            config.pageId,
          igAccountId:       config.igAccountId || undefined,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    <div style={S.card}>
                      ...
                      <Sel label="Cobro (billing event)" value={config.billingEvent} onChange={v => setConfig(c => ({ ...c, billingEvent: v }))}
                        options={[
                          { value: 'IMPRESSIONS',  label: 'CPM — Por impresiones (default)' },
                          { value: 'LINK_CLICKS',  label: 'CPC — Por clic' },
                        ]} />
                    </div>
