...
const qColor  = { good: '#22C55E', ok: '#F59E0B', bad: '#EF4444', empty: '#1A4080' }
const qBg     = { good: '#22C55E22', ok: '#F59E0B22', bad: '#EF444422', empty: '#071428' }
const qBorder = { good: '#22C55E50', ok: '#F59E0B50', bad: '#EF444450', empty: '#1A4080' }

// ─── shared table styles ─────────────────────────────────────────────────────
const TH: any  = { padding: '7px 8px', color: '#7A90AA', fontSize: '10px', fontWeight: 600, borderBottom: '1px solid #1A4080', whiteSpace: 'nowrap', textTransform: 'uppercase', letterSpacing: '0.04em', backgroundColor: '#050F1E', textAlign: 'right' }
const THL: any = { ...TH, textAlign: 'left' }
const TD: any  = { padding: '7px 8px', fontSize: '11px', borderBottom: '1px solid #07142880', textAlign: 'right', verticalAlign: 'middle' }
...
const CARD = { backgroundColor: '#071428', border: '1px solid #1A4080', borderRadius: '12px', overflow: 'hidden', marginBottom: '20px' }
...
export default async function DecisionesPage({ searchParams }: { searchParams: Promise<{ days?: string; from?: string; to?: string }> }) {
  ...
  function SectionHeader({ icon, title, sub }: { icon: string; title: string; sub: string }) {
    return (
      <div style={{ padding: '16px 20px 14px', borderBottom: '1px solid #1A4080', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={CARD}>
              ...
              {campComparison.length > 0 && (
                <div style={{ padding: '0 20px 0' }}>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: '#A8BCD0', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px', paddingTop: '12px', borderTop: '1px solid #1A4080' }}>
                    Nivel 2 — Por campaña
                  </div>
